...
throttle_adc_min = 2000 # this is a value that should be a bit superior than the min value, so if throttle is in rest position, motor will not run
throttle_adc_max = 61940 # this is a value that should be a bit lower than the max value, so if throttle is at max position, the calculated value of throttle will be the max
...
motor_min_current_start = 2.0 # to much lower value will make the motor vibrate and not run, so, impose a min limit (??)
motor_max_current_limit = 6.95 # max value, be careful to not burn your motor

motor_control_scheme = MotorControlScheme.CURRENT
# motor_control_scheme = MotorControlScheme.SPEED_CURRENT

input_control_mode = InputControlMode.PAS_MODE
...
pas_sensor = pas_sensor.PAS(
    board.D17)

brake_sensor = brake.Brake(
   board.D18) # brake sensor pin
...
vesc = vesc.Vesc(
    board.TX, # UART TX pin that connect to VESC
...
def print_data_to_terminal():
    ...
    # print(f"{(esp32.temperature_x10 / 10.0):3.1f}")
    print(f"ADC {throttle.adc_value:6} | THROTTLE POS {(throttle.value / 10.0):2.1f} % | BRAKE {brake_sensor.value:1} | CPU TEMP {(esp32.temperature_x10 / 10.0):3.1f} | MOTOR REQUESTED CURRENT {system_data.motor_target_current:2.1f} | MODE {pas_sensor.value}", end='\r\n')
...
async def task_control_motor():
    while True:
        ...
        if input_control_mode == InputControlMode.PAS_MODE:
            
            if pas_sensor.value == True:
                motor_target_current = motor_max_current_limit
            else:
                motor_target_current = 0
                    
        elif input_control_mode == InputControlMode.THROTTLE_MODE:
